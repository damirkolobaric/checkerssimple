...
class Renderer: NSObject {
    ...
    func startCameraIntro() {
        ...
        cameraStartPosition = SIMD3<Float>(3.5, 25, 3.5)
        cameraStartRotation = Float.pi * 0.25
        cameraTargetPosition = SIMD3<Float>(3.5, 12, 3.5)
        cameraTargetRotation = -Float.pi * 0.25
    ...
    func draw(in view: MTKView) {
        ...
        for (index, piece) in pieces.enumerated() {
            ...
            if let captureProgress = captureAnimationProgress[index] {
                ...
                let rotationMatrix = simd_float4x4(rotationAngle)
